...
`session/prompt` POST 的成功响应可能是带 JSON-RPC envelope 的 `200`，也可能只确认接收而由 SSE 返回 response envelope；Rust binding 必须合并这两条通道并按 request ID 去重。收到结构化 POST response 后仍应给 SSE 一个有界窗口来接收因果上先发生的 session update 和镜像 response；上游未及时镜像时使用同一 request ID 的 POST response 收口，不能无限等待。`stopReason=end_turn` 或等价正常结束可进入结果导出；取消、预算、拒绝、adapter exit 和未知 stop reason 必须显式映射，不能都当作成功。最终 text 来自当前 turn 的 agent message 聚合，不能从进程 stdout 随意截取。
