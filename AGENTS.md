...
## TypeScript SDK practices

- `sdk/typescript` is the first-party Node.js client. Keep it Node 22, ESM/NodeNext, strict TypeScript, pnpm-managed, and free of runtime dependencies unless a reviewed requirement justifies one.
- `api/openapi.yaml` remains the type source of truth. `sdk/typescript/src/schema.ts` is generated by `openapi-typescript`; never edit it manually. OpenAPI changes must regenerate it and pass `pnpm run check`.
- Keep transport code small and provider-agnostic. The local SDK may know UDS HTTP, durable idempotency, SSE resume, stable API errors, and Submission lifecycle; it must not know Tunascope business states, Git semantics, provider credentials, or concrete Source selection.
- The current SDK is local-UDS-only. Add HTTPS, bearer token, or mTLS as an explicit transport with focused tests; never silently fall back from authenticated TLS to plain TCP.
- Never log request bodies, prompts, response bodies, authorization headers, proxy values, or workspace contents. AbortSignal cancellation stops local waiting only; server cancellation always uses the explicit cancel endpoint.
- SDK tests use a fake UDS HTTP server and must cover fragmented SSE, duplicate events, reconnects, aborts, idempotency, and stable error decoding. Live-model smoke tests remain opt-in.
...
When the TypeScript SDK changes, also run:
